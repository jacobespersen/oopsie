"""Tests for GET /orgs/{org_slug}/projects/{project_id}/errors/{error_id}."""
...
async def test_error_show_404_wrong_project(
    ...
    project = await factory(ProjectFactory, user_id=current_user.id, organization_id=organization.id)
    other_project = await factory(ProjectFactory, user_id=current_user.id, organization_id=organization.id)

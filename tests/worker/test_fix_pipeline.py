...
def _setup_happy_path_mocks(mock_gh, mock_claude, mock_tempfile):
    """Wire up standard happy-path mock returns for pipeline tests."""
...
@pytest.mark.asyncio
async def test_pipeline_passes_occurrence_context_to_claude(
    ...
    with (
        ...
        _setup_happy_path_mocks(mock_gh, mock_claude, mock_tempfile)
        await run_fix_pipeline({}, str(error.id), str(project.id))
